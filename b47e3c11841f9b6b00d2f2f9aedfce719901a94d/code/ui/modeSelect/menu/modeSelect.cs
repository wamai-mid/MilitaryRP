...
public partial class modeSelect : Panel
{
	...
	public modeSelect()
	{
		...
		AddClass( "menuopen" );
		Add.Label( "Выберите фракцию:" );
		Inner = Add.Panel( "inner" );

		pvp = Inner.Add.Button( "🔴", () => PVP() );
		pvp.Add.Label( "ОДКБ", "Организация Договора о коллективной безопасности"  );
		pvpLock = pvp.Add.Label( "" );
		build = Inner.Add.Button( "🔵", () => BUILD() );
		build.Add.Label( "NATO", "Организа́ция Североатланти́ческого догово́ра"  );
		buildLock = build.Add.Label( "" );

		close = Inner.Add.Button( "❌", () => CLOSE() );
		close.Add.Label( "Закрыть", "TEXT" );
