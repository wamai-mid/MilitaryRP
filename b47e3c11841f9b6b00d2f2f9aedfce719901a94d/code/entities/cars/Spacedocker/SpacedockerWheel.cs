...
struct SpacedockerWheel
{
	private readonly SpacedockerEntity _parent;

	private float _previousLength;
	private float _currentLength;

	public SpacedockerWheel( SpacedockerEntity parent )
	{
		_parent = parent;
		_previousLength = 0;
		_currentLength = 0;
	...
	public bool Raycast( float length, bool doPhysics, Vector3 offset, ref float wheel, float dt )
	{
		...
		if ( !doPhysics && SpacedockerEntity.debug_car )
		{
		...
		var springForce = body.Mass * 30.0f * _currentLength;
